...
export default function App(props) {
  ...
  const data = useFetch({ url: "family/media" });
  const [morestate, setmorestate] = useState();

  let col = [
    { name: "title", label: "Title", w: "auto" },
...
function MoreComponent({ morestate, setmorestate, model, modelinfo, onDelete, ...props }) {
  ...
  return (
    ...
    >
      <UI.ListItemButton onClick={() => router.push(`config/media/${morestate.id}`)}>
